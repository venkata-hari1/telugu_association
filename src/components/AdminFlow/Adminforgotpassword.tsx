...
import { forgetPassword } from "../../fetures/auth/authSlice";

const Adminforgotpassword = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const { forgotMsg, error } = useSelector((state: any) => state.login);
  ...
  const handleResetPassword = () => {
    if (!email) return;
    dispatch<any>(forgetPassword({ email }))
      .then((result: any) => {
        if (result.meta.requestStatus === 'fulfilled') {
          showToast(true, result.payload.message || 'OTP Sent');
          navigate('/adminotp', { state: { email } });
        } else {
          showToast(false, result.payload || 'Failed to send OTP');
        }
      });
  };
  ...
  useEffect(() => {
    if (forgotMsg) showToast(true, forgotMsg);
